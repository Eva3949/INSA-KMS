...
public class DiscussionMediaService {
    ...
    public ResponseEntity<Resource> streamMediaContent(UUID discussionId, UUID mediaId, String username) {
        ...
        try {
            Path physicalPath = storageService.getStorageLocation().resolve(att.getObjectKey()).normalize();
            if (!Files.exists(physicalPath) || !Files.isReadable(physicalPath)) {
                // Fallback to storageService.retrieve
                InputStream is = storageService.retrieve(att.getObjectKey());
            ...
            return ResponseEntity.ok()
                    .contentLength(att.getFileSizeBytes())
                    .contentType(MediaType.parseMediaType(att.getMimeType()))
                    .header("Content-Disposition", "inline; filename=\"" + att.getOriginalFilename() + "\"")
                    .body(resource);
        } catch (Exception e) {
